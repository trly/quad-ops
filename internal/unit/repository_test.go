...
func TestCreate(t *testing.T) {
	...
	mock.ExpectExec(`INSERT INTO units`).
		WithArgs(unit.Name, unit.Type, unit.SHA1Hash, unit.CleanupPolicy, unit.UserMode, unit.RepositoryID).
...
func TestFindAll(t *testing.T) {
	...
	mock.ExpectQuery("SELECT id, name, type, sha1_hash, cleanup_policy, user_mode, repository_id, created_at FROM units").
		WillReturnRows(sqlmock.NewRows([]string{"id", "name", "type", "sha1_hash", "cleanup_policy", "user_mode", "repository_id", "created_at"}).
...
func TestFindByUnitType(t *testing.T) {
	...
	mock.ExpectQuery("SELECT id, name, type, sha1_hash, cleanup_policy, user_mode, repository_id, created_at FROM units WHERE type = ?").
		WithArgs(unitType).
		WillReturnRows(sqlmock.NewRows([]string{"id", "name", "type", "sha1_hash", "cleanup_policy", "user_mode", "repository_id", "created_at"}).
			AddRow(expectedUnits[0].ID, expectedUnits[0].Name, expectedUnits[0].Type,
				expectedUnits[0].SHA1Hash, expectedUnits[0].CleanupPolicy, expectedUnits[0].UserMode, expectedUnits[0].RepositoryID, expectedUnits[0].CreatedAt))
...
func TestFindById(t *testing.T) {
	...
	// Expect SELECT query with WHERE clause and repository_id
	mock.ExpectQuery("SELECT id, name, type, sha1_hash, cleanup_policy, user_mode, repository_id, created_at FROM units WHERE id = ?").
		WithArgs(unitID).
		WillReturnRows(sqlmock.NewRows([]string{"id", "name", "type", "sha1_hash", "cleanup_policy", "user_mode", "repository_id", "created_at"}).
			AddRow(expectedUnit.ID, expectedUnit.Name, expectedUnit.Type,
				expectedUnit.SHA1Hash, expectedUnit.CleanupPolicy, expectedUnit.UserMode, expectedUnit.RepositoryID, expectedUnit.CreatedAt))
