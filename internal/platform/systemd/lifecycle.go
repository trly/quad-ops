...
// It checks if the unit file exists via D-Bus GetUnitProperties with exponential backoff
...
func (l *Lifecycle) waitForUnitGeneration(ctx context.Context, serviceName string) error {
	...
	for {
		...
		// Try to get unit properties - if successful, unit exists
		_, err = conn.GetUnitProperties(ctx, serviceName)
		_ = conn.Close()

		if err == nil {
			// Unit exists and was generated
