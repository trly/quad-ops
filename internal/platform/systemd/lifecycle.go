...
func (l *Lifecycle) StartMany(ctx context.Context, names []string) map[string]error {
	...
	// After verifying all units exist, add a buffer to ensure systemd has fully
	...
	// The 2s delay allows systemd's unit generator to fully process all units.
...
func (l *Lifecycle) Exists(ctx context.Context, name string) (bool, error) {
	...
	props, err := conn.GetUnitProperties(ctx, name)
	if err != nil {
		return false, nil
	}

	// Check LoadState - "not-found" means unit doesn't exist
...
// It checks if the unit is in the "loaded" state via D-Bus with exponential backoff
// (starting at 50ms, doubling each retry, up to unitGenerationTimeout).
// Units must be fully loaded by systemd before they can be started/stopped/restarted.
func (l *Lifecycle) waitForUnitGeneration(ctx context.Context, serviceName string) error {
	...
	for {
		...
		// Get unit properties - must check LoadState to ensure systemd has fully processed it
		props, err := conn.GetUnitProperties(ctx, serviceName)
		_ = conn.Close()

		if err == nil {
			// Check if the unit is in "loaded" state (not just existing)
			if loadState, ok := props["LoadState"].(string); ok && loadState == "loaded" {
				// Unit is fully loaded and accessible
				l.logger.Debug("Unit generated successfully", "unit", serviceName, "attempts", attempt)
				return nil
			}
			// Unit exists but not fully loaded yet, retry
			loadStateStr := "unknown"
			if loadState, ok := props["LoadState"].(string); ok {
				loadStateStr = loadState
			}
			l.logger.Debug("Unit exists but not fully loaded yet, retrying",
				"unit", serviceName,
				"attempt", attempt,
				"loadState", loadStateStr)
		}

		// Check if we've exceeded the timeout
		if time.Now().Add(backoff).After(deadline) {
			l.logger.Error("Unit generation timeout", "unit", serviceName, "timeout", maxWait.String(), "attempts", attempt)
			return fmt.Errorf("unit %s failed to reach 'loaded' state after %s (%d retries)", serviceName, maxWait.String(), attempt)
		}

		// Log retry attempt at debug level
		l.logger.Debug("Unit not yet loaded, retrying",
			"unit", serviceName,
			"attempt", attempt,
